...
import { BookOpen, ClipboardList, PenLine, RotateCcw, Trash2 } from 'lucide-react'
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="border-b border-border bg-card/80 backdrop-blur-sm"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-5 sm:px-8"><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl bg-primary text-primary-foreground"><BookOpen aria-hidden="true" /></div><div><p className="text-base font-bold tracking-tight">HGE CELE</p><p className="text-xs text-muted-foreground">Terms review room</p></div></div><div className="flex items-center gap-2 rounded-xl bg-secondary p-1"><button type="button" onClick={() => setMode('quiz')} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold transition ${mode === 'quiz' ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><ClipboardList aria-hidden="true" />Quiz</button><button type="button" onClick={() => setMode('build')} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold transition ${mode === 'build' ? 'bg-background shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><PenLine aria-hidden="true" />Build quiz</button></div></div></header>
